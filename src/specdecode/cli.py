...
def _toy_decoder(seed: int) -> SpeculativeDecoder:
    draft = TableModel(
...
def run_demo(seed: int) -> int:
    result = _toy_decoder(seed).generate([0])
...
def run_generate(args: argparse.Namespace) -> int:
    ...
    decoder = SpeculativeDecoder(pair.draft, pair.target, config, rng=random.Random(args.seed))
    ...
    print(
        ...
        f"{result.stats.verification_rounds} verification rounds",
        file=sys.stderr,
    )
    return 0
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command in (None, "demo"):
        return run_demo(getattr(args, "seed", 7))
    if args.command == "generate":
        from .huggingface import MissingOptionalDependencyError
